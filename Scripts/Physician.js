$(function () {
    ...
    var WatchlistView = Backbone.View.extend({
        ...
        events: {
            'click .removewatch': 'RemoveWatch'
        },
        RemoveWatch: function (e) {
            var clickme = $(e.currentTarget);
            var patid = clickme.attr("data-id");
            $.ajax({
                url: 'http://localhost:56393/api/WatchLists?userid=' + patid,
                type: 'Delete',
    ...
    var ConversationView = Backbone.View.extend({
        ...
        events: {
            'click .given-name': 'sendmessage'
        },
        sendmessage: function (e) {
            var clickme = $(e.currentTarget);
            var sendTo = clickme.attr("id");
            $(".patient-record--nav-item--messages").find("a").click();
    ...
    var PatientHeadwatch = Backbone.Model.extend({
    ...
    var PatientHeadwatchCollection = Backbone.Collection.extend({

        model: PatientHeadwatch,
    ...
    var PatientHeadwatchView = Backbone.View.extend({
        el: ".patient-record-header",
        template: _.template($('#PatientWatchHeadertemplate').html()),
        events: {
            'click .addtowatch': 'addToWatchlist'
        },
    ...
    var Appointment = Backbone.Model.extend({
        url: function () {
            return "http://localhost:56393/api/Appointment/" + this.get("AppointmentsID");
        },
    ...
    var AppointmentsCollection = Backbone.Collection.extend({
        ...
        loadByAppointments: function (userid, callback) {
            this.fetch({
    ...
    var AppointmentsView = Backbone.View.extend({
        el: "#patient-Appointmnets",
        template: _.template($('#patappointmenttemplate').html()),
        initialize: function () {
            var thisview = this;
            this.options.model.bind('reset', function () {
                thisview.render();
        ...
        render: function (eventName) {
            $('#patient-Appointmnets').find('.patient-record-table--header').remove();
            _.each(this.model.models, function (patappointment) {
                var patappointmenttemplate = this.template(patappointment.toJSON());
                $(this.el).append(patappointmenttemplate);
            }, this);
            return this;
        }
    ...
    var RouterMain = Backbone.Router.extend({
        ...
        AdmissionList: function (userid) {
            var patientdetails = new PatientHeadwatchCollection();
            patientdetails.loadByPatient(userid, function () { });
            var patientheadwatchview = new PatientHeadwatchView({ model: patientdetails });
            ...
            var patappointments = new AppointmentsCollection();
            patappointments.loadByAppointments(userid, function () { });
            var patappointmentview = new AppointmentsView({ model: patappointments });
        },
